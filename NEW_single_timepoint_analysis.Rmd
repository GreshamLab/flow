...
sample.param <- data.frame(SAMPLE=NA, WELL=NA, STRAIN=NA, PLOIDY=NA, MEDIA=NA, EXPERIMENT=NA)
...
for(i in 1:length(filteredData)){
  ...
  temp.row <- cbind(SAMPLE=sample, WELL=well, STRAIN=strain, GENOTYPE=genotype, PLOIDY=ploidy, MEDIA=media,  EXPERIMENT=exp)
  if(extract.FSC_A == "Yes"){temp.row<-cbind(temp.row, FSC.A=fsc.a)}
  if(extract.SSC_A == "Yes"){temp.row<-cbind(temp.row, SSC.A=ssc.a)}
  if(extract.FL1_A == "Yes"){temp.row<-cbind(temp.row, FL1.A=fl1.a)}
  if(extract.FL2_A == "Yes"){temp.row<-cbind(temp.row, FL2.A=fl2.a)}
  if(extract.FL3_A == "Yes"){temp.row<-cbind(temp.row, FL3.A=fl3.a)}
  if(extract.FL4_A == "Yes"){temp.row<-cbind(temp.row, FL4.A=fl4.a)}
  if(extract.FSC_H == "Yes"){temp.row<-cbind(temp.row, FSC.H=fsc.h)}
  if(extract.SSC_H == "Yes"){temp.row<-cbind(temp.row, SSC.H=ssc.h)}
  if(extract.FL1_H == "Yes"){temp.row<-cbind(temp.row, FL1.H=fl1.h)}
  if(extract.FL2_H == "Yes"){temp.row<-cbind(temp.row, FL2.H=fl2.h)}
  if(extract.FL3_H == "Yes"){temp.row<-cbind(temp.row, FL3.H=fl3.h)}
  if(extract.FL4_H == "Yes"){temp.row<-cbind(temp.row, FL4.H=fl4.h)}
  if(extract.Width == "Yes"){temp.row<-cbind(temp.row, Width=width)}
  if(extract.Time == "Yes"){temp.row<-cbind(temp.row, Time=time)}
  
  filtered.data <- rbind(filtered.data, temp.row)
 ...
 #Cleans up DataFrames
  ...
  filtered.data$EXP<-as.factor(filtered.data$EXPERIMENT)
  ...
  if(extract.Width == "Yes"){cbind(filtered.data, Width=width)}
  if(extract.Time == "Yes"){cbind(filtered.data, Time=time)}
