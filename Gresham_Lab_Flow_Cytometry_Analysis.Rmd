...
```{r unpack}
#source("src/combineFlowSets.R");tmp<-combineFlowSets(list(ingate,outgate))
...
#  filteredData <- combineFlowSets(list(ingate,outgate))
#}
...
```{r summaryPlotz}
fdatar <- fs2df(filteredDatar)
head(fdatar)
mfdatar <- melt(fdatar,
  id.vars=c("Strain","ul500nMDye","Ploidy","Media","name"),
...
```{r filteredSummarys}
sumz <- fsApply(filteredDatar,function(x){
...
```{r}
aggregate(data=fdatar,FL1.A~ul500nMDye,FUN=median)
medianFL1Anodye <- median(subset(fdatar,ul500nMDye==0)$FL1.A)
ggplot(fdatar)+
...
```{r}
aggregate(data=fdatar,FSC.A~ul500nMDye,FUN=median)
medianFSCAnodye <- median(subset(fdatar,ul500nMDye==0)$FSC.A)
ggplot(fdatar)+
